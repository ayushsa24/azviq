import { getServerSession } from "next-auth";
import { authOptions } from "@/lib/auth";
import Navbar from "@/components/landingpage/Navbar";
import Footer from "@/components/landingpage/Footer";
import PrepScrollingSection from "@/components/landingpage/PrepScrollingSection";

export const metadata = {
  title: "Preparation Feature — Azviq",
  description: "AI-generated exercises, flashcards, active recall reviews, and an AI Study Teacher.",
};

export default async function PrepFeaturePage() {
  const session = await getServerSession(authOptions);

  return (
    <div className="min-h-screen bg-[#F4F4F6] text-[#1D1D1F]" style={{ fontFamily: "'Inter',system-ui,sans-serif" }}>
      <Navbar isLoggedIn={!!session} />
      
      <main className="pt-24 pb-12">
        <PrepScrollingSection />
      </main>

      <Footer />
    </div>
  );
}
