...
function SignupForm() {
  ...
  const handleProfileChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    // Prevent XSS / HTML tags by stripping <, >, [, ], { }
    if (name !== 'avatar_url') {
      value = value.replace(/[<>{}[\]]/g, '');
    }

    if (name === "mobile_no") {
      // Only allow digits and maybe a plus sign at the start
      value = value.replace(/(?!^\+)[^\d]/g, '');
      if (value.length > 15) return; // limit length
    }
    
    if (name === "username") {
      // Username shouldn't have spaces or weird characters
      value = value.replace(/[^a-zA-Z0-9_]/g, '');
    }

    if (name === "bio") {
      const wordCount = value.trim().split(/\s+/).filter(Boolean).length;
      if (wordCount > 20 && value.length > (profileForm.bio || "").length) {
        return; // Prevent typing more if already at 20 words
      }
    }
              ...
              <div>
                ...
                <div className="relative">
                  <textarea
                    ...
                    onChange={handleProfileChange}
                    rows={2}
                    className="w-full px-3.5 py-2.5 pb-7 rounded-xl border text-sm outline-none transition-all resize-none
                      bg-white dark:bg-[#1c1c1c]
                      text-[#1a1a1a] dark:text-white
                      placeholder-[#BABABA] dark:placeholder-[#545454]
                      border-[#E8E5E0] dark:border-[#2e2e2e]
                  ...
                  <span className={`absolute bottom-2 right-3 text-[10px] font-medium ${
                    profileForm.bio.trim().split(/\s+/).filter(Boolean).length >= 20 
                      ? "text-[#C2A27A] dark:text-[#D2B48C]" 
                      : "text-[#BABABA] dark:text-[#545454]"
                  }`}>
                    {profileForm.bio.trim().split(/\s+/).filter(Boolean).length}/20 words
                  </span>
                </div>
              </div>{/* end Bio */}
              </div>{/* end space-y-3 */}
