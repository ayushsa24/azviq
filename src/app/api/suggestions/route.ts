...
// GET — generate and return AI suggestions for the authenticated user
export async function GET() {
    try {
        ...
        // 1. Daily Study Target suggestion
        const totalMinutes =
            (dailySummary as { total_minutes?: number } | null)?.total_minutes || 0;
        if (totalMinutes < 60) {
            suggestions.push({
        ...
        // 2. Weak Topics from low-scoring exercises
        ...
        // 3. Spaced Revision for old notes
...
// PATCH — update a suggestion item's status
export async function PATCH(req: Request) {
    try {
        ...
        if (!itemId || !status) {
            return apiError("itemId and status are required", 400, "BAD_REQUEST");
        }
