...
function renderInline(text: string, isDark: boolean) {
    ...
    return parts.map((part, j) =>
        part.startsWith("**") && part.endsWith("**")
            ? <strong key={j} className={isDark ? "text-[#CFCFCF] font-semibold" : "text-[#252525] font-semibold"}>{part.slice(2, -2)}</strong>
...
function SummaryRenderer({ text, isDark }: { text: string; isDark: boolean }) {
                ...
                <table className="w-full text-xs border-collapse">
                    <thead>
                        <tr className={isDark ? "bg-[#252525]" : "bg-[#F5F3EF]"}>
                            {header.map((h, hi) => (
                                <th key={hi} className={`px-3 py-2 text-left font-semibold border-b ${isDark ? "border-[#333] text-[#CFCFCF]" : "border-[#E8E5E0] text-[#252525]"}`}>
                    ...
                    <tbody>
                        {rows.map((r, ri) => (
                            <tr key={ri} className={ri % 2 === 0 ? (isDark ? "bg-[#1A1A1A]" : "bg-white") : (isDark ? "bg-[#1E1E1E]" : "bg-[#F9F8F6]")}>
                                {r.map((cell, ci) => (
                                    <td key={ci} className={`px-3 py-2 ${isDark ? "text-[#B0B0B0]" : "text-[#3A3A3A]"} border-b ${isDark ? "border-[#2E2E2E]" : "border-[#F0EDE8]"}`}>
    ...
    while (i < lines.length) {
        ...
        if (line.startsWith("## ")) {
            elements.push(<h2 key={i} className={`text-lg font-extrabold mt-5 first:mt-0 pb-1 border-b ${isDark ? "text-[#CFCFCF] border-[#2E2E2E]" : "text-[#161514] border-[#E8E5E0]"}`}>{line.replace(/^## /, "")}</h2>);
        } else if (line.startsWith("### ")) {
            elements.push(<h3 key={i} className={`text-sm font-bold mt-3 ${isDark ? "text-[#CFCFCF]" : "text-[#252525]"}`}>{line.replace(/^### /, "")}</h3>);
        } else if (line.trimStart().startsWith("- ") || line.trimStart().startsWith("• ")) {
            const content = line.trimStart().replace(/^[-•]\s+/, "");
            elements.push(
                <div key={i} className="flex items-start gap-2">
                    <span className={`mt-1.5 w-1.5 h-1.5 rounded-full shrink-0 ${isDark ? "bg-[#545454]" : "bg-[#CFCFCF]"}`} />
                    <p className={`text-sm leading-relaxed ${isDark ? "text-[#B0B0B0]" : "text-[#3A3A3A]"}`}>{renderInline(content, isDark)}</p>
                </div>
            );
        } else if (line.trim() === "") {
            elements.push(<div key={i} className="h-2" />);
        } else {
            elements.push(<p key={i} className={`text-sm leading-relaxed ${isDark ? "text-[#B0B0B0]" : "text-[#3A3A3A]"}`}>{renderInline(line, isDark)}</p>);
        }
        i++;
    }
        ...
        <div className="flex flex-col h-full bg-[#F5F3EF] dark:bg-[#1A1A1A] overflow-hidden">
            ...
            <div className={`flex items-center gap-3 px-4 sm:px-6 py-3 border-b shrink-0 ${isDark ? "border-[#2E2E2E] bg-[#161616]" : "border-[#E8E5E0] bg-white"}`}>
                ...
                <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-[#252525] dark:text-[#CFCFCF] truncate">{revision.title}</p>
                    <p className="text-[10px] text-[#7D7D7D] flex items-center gap-1">
            ...
            {/* Tab bar */}
            <div className={`flex border-b shrink-0 px-4 sm:px-6 ${isDark ? "border-[#2E2E2E] bg-[#161616]" : "border-[#E8E5E0] bg-white"}`}>
                ...
                <button className={tabCls("keywords")} onClick={() => setActiveTab("keywords")}>
                    <span className="flex items-center gap-1.5"><Key size={13} /> Keywords <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ml-1 ${isDark ? "bg-[#333] text-[#7D7D7D]" : "bg-[#F0EDE8] text-[#545454]"}`}>{kw.length}</span></span>
                </button>
                <button className={tabCls("qa")} onClick={() => { setActiveTab("qa"); setQaIndex(0); setShowAnswer(false); }}>
                    <span className="flex items-center gap-1.5"><HelpCircle size={13} /> Q&amp;A <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ml-1 ${isDark ? "bg-[#333] text-[#7D7D7D]" : "bg-[#F0EDE8] text-[#545454]"}`}>{qa.length}</span></span>
                </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto scrollbar-hide">
                ...
                {activeTab === "summary" && (
                    <div className="max-w-3xl mx-auto px-4 sm:px-8 py-6">
                        <div className={`p-5 sm:p-6 rounded-2xl border ${isDark ? "bg-[#1E1E1E] border-[#2E2E2E]" : "bg-white border-[#E8E5E0] shadow-sm"}`}>
                            <h2 className={`text-xs font-bold uppercase tracking-widest mb-5 ${isDark ? "text-[#7D7D7D]" : "text-[#545454]"}`}>Revision Summary</h2>
                ...
                {activeTab === "keywords" && (
                    <div className="max-w-3xl mx-auto px-4 sm:px-8 py-6 flex flex-col gap-2">
                        <p className="text-[11px] text-[#7D7D7D] mb-1">Click a keyword to expand its definition.</p>
                        {kw.length === 0 && (
                        ...
                        {kw.map((k, i) => (
                            ...
                            >
                                <div className="flex items-center justify-between gap-2">
                                    <span className="text-sm font-semibold text-[#252525] dark:text-[#CFCFCF]">{k.term}</span>
                                    {expandedKw === i ? <ChevronUp size={14} className="text-[#7D7D7D] shrink-0" /> : <ChevronDown size={14} className="text-[#7D7D7D] shrink-0" />}
                                </div>
                                {expandedKw === i && (
                                    <p className="text-sm text-[#545454] dark:text-[#7D7D7D] mt-2 leading-relaxed border-t border-[#E8E5E0] dark:border-[#2E2E2E] pt-2">
                                        {k.definition}
                                    </p>
                                )}
                            </button>
                            ...
                            <>
                                ...
                                <div className="flex items-center justify-between text-xs text-[#7D7D7D]">
                                    ...
                                    <div className={`flex-1 mx-4 h-1 rounded-full ${isDark ? "bg-[#2E2E2E]" : "bg-[#E8E5E0]"} overflow-hidden`}>
                                        <div
                                            className="h-full bg-[#252525] dark:bg-[#CFCFCF] rounded-full transition-all"
                                ...
                                <div className={`p-6 rounded-2xl border ${isDark ? "bg-[#1E1E1E] border-[#2E2E2E]" : "bg-white border-[#E8E5E0] shadow-sm"}`}>
                                    <p className={`text-[10px] font-bold uppercase tracking-widest mb-3 ${isDark ? "text-[#545454]" : "text-[#9E9E9E]"}`}>Question</p>
                                    <p className="text-base font-semibold leading-relaxed text-[#252525] dark:text-[#CFCFCF]">
                                        {currentQA.question}
                                    </p>
                                </div>
                                ...
                                {!showAnswer ? (
                                    <button
                                        ...
                                        className={`w-full py-3 rounded-xl border-2 border-dashed text-sm font-semibold transition-all flex items-center justify-center gap-2 ${isDark
                                            ? "border-[#333] text-[#7D7D7D] hover:border-[#545454] hover:text-[#CFCFCF]"
                                ...
                                <div className={`flex items-center ${qaIndex > 0 ? "justify-between" : "justify-end"}`}>
                                    {qaIndex > 0 && (
                                        <button
                                            ...
                                            className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium border transition-all ${isDark ? "border-[#333] text-[#7D7D7D] hover:bg-[#252525] hover:text-[#CFCFCF]" : "border-[#E8E5E0] text-[#545454] hover:bg-[#F0EDE8]"}`}
                                        >
                                    ...
                                    {qaIndex < qa.length - 1 ? (
                                        <button
                                            onClick={() => { setQaIndex(i => i + 1); setShowAnswer(false); }}
                                            className={`flex items-center gap-2 px-5 py-2 rounded-full text-sm font-medium transition-all ${isDark ? "bg-[#CFCFCF] text-[#252525] hover:bg-white" : "bg-[#252525] text-white hover:bg-[#1A1A1A]"}`}
                                        >
                                        ...
                                        <button
                                            ...
                                            className={`flex items-center gap-2 px-5 py-2 rounded-full text-sm font-semibold transition-all ${isDark ? "bg-[#CFCFCF] text-[#252525] hover:bg-white" : "bg-[#252525] text-white hover:bg-[#1A1A1A]"}`}
                                        >
                                            Start Over
                                        </button>
                                    )}
