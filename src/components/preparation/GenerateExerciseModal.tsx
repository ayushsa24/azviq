            ...
            <div className={`py-4 ${isDark ? 'text-[#CFCFCF]' : 'text-[#252525]'}`}>
                ...
                {step === "select" && (
                    <div className="space-y-5">
                        <div className={`flex items-center gap-3 p-4 rounded-xl border ${isDark ? 'bg-[#252525] border-[#545454]' : 'bg-[#F0EDE8] border-[#DEDBD6]'}`}>
                            ...
                            <div>
                                <h3 className="font-semibold text-sm text-[#252525] dark:text-[#CFCFCF]">New Exercise</h3>
                                <p className="text-xs text-[#545454] dark:text-[#7D7D7D]">Select a note — AI will generate a custom quiz.</p>
                            </div>
                        </div>

                        <div>
                            <label className="block text-xs font-semibold mb-2 uppercase tracking-widest text-[#545454] dark:text-[#7D7D7D]">Source Material</label>
                            <div className="relative">
                                <select
                                    className={`w-full appearance-none px-4 py-3 pr-10 rounded-xl border outline-none font-medium text-sm transition-all focus:border-[#7D7D7D] ${isDark
                                        ? 'bg-[#1A1A1A] border-[#545454] text-[#CFCFCF]'
                            ...
                            <p className="text-xs text-[#7D7D7D] mt-2">AI reads this document and crafts targeted questions.</p>
                        </div>

                        {/* Question Count Selector */}
                        <div className="mb-6">
                            <div className="flex items-center justify-between mb-2">
                                <label className="block text-xs font-semibold uppercase tracking-widest text-[#545454] dark:text-[#7D7D7D]">Quiz Length</label>
                                <span className={`text-xs font-bold px-2 py-0.5 rounded-lg ${isDark ? 'bg-[#333] text-[#CFCFCF]' : 'bg-[#F0EDE8] text-[#252525]'}`}>{questionCount} Questions</span>
                            </div>
                                ...
                                {[5, 10, 15, 20].map((num) => (
                                    <button
                                        ...
                                        className={`flex-1 py-2.5 rounded-xl text-xs font-bold border transition-all active:scale-95 ${questionCount === num
                                            ...
                                            : isDark ? 'bg-transparent border-[#333] text-[#7D7D7D] hover:bg-[#252525]' : 'bg-white border-[#E8E5E0] text-[#545454] hover:bg-[#F0EDE8]'
                                            }`}
                                    >
                                        {num}
                                    </button>
                ...
                {step === "generating" && (
                    <div className="flex flex-col items-center justify-center py-14 text-center space-y-5">
                        <div className="relative w-20 h-20">
                            ...
                            <div className={`absolute inset-0 rounded-full border-4 border-t-transparent animate-spin ${isDark ? 'border-[#CFCFCF]' : 'border-[#252525]'}`} />
                            <div className="absolute inset-0 flex items-center justify-center">
                                <Sparkles className="w-7 h-7 text-[#545454] dark:text-[#7D7D7D] animate-pulse" />
                            </div>
                        </div>
                        <div>
                            <h3 className="text-base font-bold mb-1 text-[#252525] dark:text-[#CFCFCF]">Generating Quiz…</h3>
                            <p className="text-sm text-[#545454] dark:text-[#7D7D7D] max-w-xs mx-auto">
                                Analysing <span className="font-semibold text-[#252525] dark:text-[#CFCFCF]">{selectedNote?.title || "your file"}</span>…
                            </p>
                        </div>
                    </div>
                )}
                ...
                {step === "result" && (
                    <div className="flex flex-col items-center text-center space-y-5 py-4">
                        ...
                        <div>
                            <h3 className="text-xl font-bold mb-1 text-[#252525] dark:text-[#CFCFCF]">Quiz Ready!</h3>
                            <p className="text-sm text-[#545454] dark:text-[#7D7D7D]">{createdExercise?.questions?.length || questionCount} questions generated from your note.</p>
                        </div>
                        <div className={`w-full p-4 rounded-xl border text-left flex items-start gap-3 ${isDark ? 'bg-[#1A1A1A] border-[#333]' : 'bg-[#F5F3EF] border-[#E8E5E0]'}`}>
                            <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${isDark ? 'bg-[#252525] border border-[#545454]' : 'bg-white border border-[#E8E5E0]'}`}>
                                <FileText className="w-4 h-4 text-[#545454] dark:text-[#7D7D7D]" />
                            </div>
                            <div>
                                <h4 className="font-semibold text-sm text-[#252525] dark:text-[#CFCFCF]">{selectedNote?.title || "Custom"} Quiz</h4>
                                <p className="text-xs text-[#7D7D7D] mt-0.5">{createdExercise?.questions?.length || questionCount} Questions · Medium Difficulty</p>
                            </div>
                        </div>
                        <button
                            onClick={() => onSuccess(createdExercise)}
                            className={`w-full py-3 rounded-xl font-semibold text-sm transition-all ${isDark ? 'bg-[#CFCFCF] text-[#252525] hover:bg-white' : 'bg-[#252525] text-white hover:bg-[#1A1A1A]'}`}
                        >
                            Start Exercise Now →
                        </button>
                    </div>
