...
export default function BottomNav({ isFullPageLayer = false }: { isFullPageLayer?: boolean }) {
  ...
  // Render bottom navigation bar flush at the bottom of the screen (no floating margins)
  const navClass = `fixed bottom-0 left-0 right-0 z-[100] border-t transition-all duration-300 ease-in-out shadow-2xl md:hidden 
     pb-[calc(env(safe-area-inset-bottom,0px)+8px)] pt-2
     ${hidden ? 'translate-y-[150%] opacity-0 pointer-events-none' : 'translate-y-0 opacity-100'}
     ${theme === 'dark' ? 'bg-[#1A1A1A] border-[#2E2E2E]' : 'bg-white/90 backdrop-blur-md border-[#E8E5E0]'}`;

  return (
    <nav className={navClass}>
      <div className="mx-auto flex justify-between items-center max-w-md px-8 py-0.5">
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`relative group flex flex-col items-center justify-center transition-all duration-300 active:scale-90 p-2 rounded-2xl
                ${theme === 'dark'
            ...
            >
              ...
              {isActive && (
                <span className={`absolute -bottom-0.5 w-1.5 h-1.5 rounded-full animate-in zoom-in duration-300 
